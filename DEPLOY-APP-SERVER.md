...
- 与 main `cb8aaa88b0e0642fa650c003d274f7c4244bdeee` 无冲突。14 项测试和语法检查通过，VPS 同样通过。
- 修复聊天历史并发读取时旧响应覆盖新响应的问题；退出时清理草稿与待处理状态，取消旧页面的联网监听；恢复网络只刷新历史，不自动重发问题。
- 在独立 VPS 数据副本上认领旧库，确认原有 3 份资料保留；新测试课件完成真实 Codex 梳理及问答，服务重启后账号与聊天记录仍在；朋友账号为空库，跨账号读课件、文件、问答及触发梳理均返回 404。未认领正式主人的账号。
- 浏览器以 390×844 和 320×740 视口配置检查（浏览器缩放影响实际 CSS 像素），输入与按钮可达、未见横向溢出或异常底部间距。模拟 API 断开后保留草稿，恢复并手动提交后成功，换账号不显示前一账号资料。AI 排版验收使用固定测试回答。
- 尚未进行真机 Safari/PWA 键盘测试，也未调用朋友的真实收费 API；不将模拟手机视口或替身接口结果记为这两项已通过。自定义背景、透明顶部不在本 PR 的功能范围内。
