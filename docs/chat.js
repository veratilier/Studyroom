...
(()=>{
  ...
  window.studyroomResetChat=()=>{pending.clear();drafts.clear();activeController?.abort();};
  window.studyroomMountChat=({courseId,target,api,isCurrent})=>{
    activeController?.abort();const controller=new AbortController();activeController=controller;let loadVersion=0;
    ...
    const alive=()=>!controller.signal.aborted&&panel.isConnected&&isCurrent();
    async function load(){
      const version=++loadVersion;
      try{const data=await api(`/courses/${courseId}/chat`);if(!alive()||version!==loadVersion)return;
      ...
      }catch(e){if(alive()&&version===loadVersion)status.textContent=e.message;}
    }
    refresh.onclick=load;
    window.addEventListener('online',()=>{if(alive())load();},{signal:controller.signal});
    form.onsubmit=async e=>{
      ...
      try{const result=await api(`/courses/${courseId}/chat`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question,request_id:request.id})});
        ...
        if(alive())await load();
      }catch(e){if(!alive())return;if(e.status&&e.status<500)pending.delete(courseId);if(alive())status.textContent=e.message+' 可刷新查看；重发同一问题不会重复创建请求。';}
