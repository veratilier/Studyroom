...
test('existing BIO101 progress survives course switching; uploaded terms use separate progress IDs',async()=>{
  ...
  const ctx=vm.createContext({console,Date,setTimeout,window:{studyroomBuiltinNotes:()=>'',addEventListener(){}},navigator:{},document:{querySelector:get,querySelectorAll:()=>[]},localStorage:{getItem:k=>stored.get(k),setItem:(k,v)=>stored.set(k,v)},fetch:async()=>({ok:true,json:async()=>JSON.parse(readFileSync(new URL('../docs/words.json',import.meta.url),'utf8'))})});
